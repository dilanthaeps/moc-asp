...
strSql = "SELECT VES.VESSEL_CODE , VES.VESSEL_NAME , TC.TIME_CHARTERER_ID , TC.SHORT_NAME TC_NAME"
strSql = strSql & "   FROM   WLS_VW_VESSELS_NEW VES , MOC_TIME_CHARTERERS TC "
strSql = strSql & "   ORDER BY VESSEL_NAME,TIME_CHARTERER_ID"
Set rsObj_moc_tc = connObj.Execute(strSql)

strSql = "SELECT   MTA.TC_VESSEL_ASGN_ID , MTA.VESSEL_CODE , MTA.TIME_CHARTERER_ID FROM MOC_TC_VESSEL_ASGN MTA"
...
<script language="VBScript" runat=server>
	function getObjValue(VESSEL_CODE,TIME_CHARTERER_ID)
	...
	rsObj_asgn.MoveFirst
		...
		if (cstr(rsObj_asgn("VESSEL_CODE")) = cstr(VESSEL_CODE) and cstr(rsObj_asgn("TIME_CHARTERER_ID"))=cstr(TIME_CHARTERER_ID)) then
		v_tem=1
...
<TITLE>Tanker Pacific - MOC - TC Assignment</TITLE>
</HEAD>
<BODY>
...
<br> 
<form name=thisform  action=ves_tc_asgn_save.asp method=post >
<table border=0>
...
if not (rsObj_moc_tc.eof or rsObj_moc_tc.bof) then 
	...
	while not rsObj_moc_tc.eof
		...
		if cstr(v_moc) <> cstr(rsObj_moc_tc("VESSEL_CODE")) then
			Response.Write chr(13)&chr(13)&"<tr><td class=tableheader colspan=" & v_no_cols & ">"&rsObj_moc_tc("VESSEL_NAME")&"-"&rsObj_moc_tc("VESSEL_CODE")&"</td></tr><tr>"
			v_moc = rsObj_moc_tc("VESSEL_CODE")
			v_ctr=1
		end if
		
		Response.Write "<td class=tabledata > <INPUT type='checkbox'   name=v_"&rsObj_moc_tc("VESSEL_CODE")&"_"&rsObj_moc_tc("TIME_CHARTERER_ID")&" value="&getObjValue(rsObj_moc_tc("VESSEL_CODE"),rsObj_moc_tc("time_charterer_id"))&" "& v_check&">"&rsObj_moc_tc("TC_NAME")&"&nbsp; </td>"
