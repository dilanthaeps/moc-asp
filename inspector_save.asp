...
%>
			<SCRIPT LANGUAGE="JavaScript">
				self.parent.opener.document.form1.action = "inspector_maint.asp?v_message=<% =v_message %>";
...
<%
	...
	Idval = Request("mode")
	if Idval = "" then
		strSql= "INSERT INTO  moc_inspectors"
		strSql = strSql & " ( inspector_id , SHORT_NAME , FULL_NAME , ADDRESS , TELEPHONE,MOBILE "
		...
		v_message = "MOC Inspector details Created Successfully"
	else
		strSql = "UPDATE  moc_inspectors  A SET "
		...
		strSql = strSql &" where a.inspector_id=" & request("inspector_id")
		v_message = "MOC Inspector details Updated Successfully"
	end if
